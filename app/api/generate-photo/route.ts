...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // ── 생성 성공 → 크레딧 1개 차감 ─────────────────────────────────
    const deductResult = await deductCredit(supabase, user.id, `photo-${Date.now()}`);
    console.log('[generate-photo] credit deduct:', deductResult);

    return NextResponse.json({ images, creditBalance: deductResult.balance });
  } catch (err: unknown) {
